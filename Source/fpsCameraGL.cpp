...
// 説明   : 初期化処理
//==============================================================================
bool CFpsCameraGL::Init( VECTOR3 posV , VECTOR3 posR )
{
	m_posV = posV;
	m_posR = posR;
...
void CFpsCameraGL::Update( void )
{
	...
	//------------------------------------------
	// 視点・注視点→回転座標への変換
	m_angleH = GetHAngle( &m_posV , &m_posR );
	m_angleV = GetVAngle( &m_posV , &m_posR );
	...
	{
		mauseVel.z < 0.0f ? m_fDistance -= 10.0f : m_fDistance += 10.0f;

		if( m_fDistance < 10.0f ){ m_fDistance = 10.0f; }
	}

	//------------------------------------------
	// [ 回転座標→３次元座標への変換 ]
	//------------------------------------------
	m_posR = VECTOR3( 0.0f , 0.0f , 0.0f );						// 注視点の初期化をしておく
...
// 説明   : 水平方向取得処理
//==============================================================================
float CFpsCameraGL::GetHAngle( VECTOR3* vPosV , VECTOR3* vPosR )
{
	// 注視点への向きベクトルを求める
	VECTOR3 vDir;
...
// 説明   : 垂直方向取得
//==============================================================================
float CFpsCameraGL::GetVAngle( VECTOR3* vPosV , VECTOR3* vPosR )
{
	// 注視点への向きベクトルを求める
	VECTOR3 vDir;
	Vec3Subtract( &vDir , vPosR , vPosV );
...
//==============================================================================
VECTOR2 CFpsCameraGL::MoveRad( VECTOR3 vel )
{
	VECTOR2 out;
...
//==============================================================================
VECTOR3 CFpsCameraGL::MoveTranslate( VECTOR3 vel )
{
